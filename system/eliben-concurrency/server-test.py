...
def server_runner(path, args, stop_event):
    ...
    """
    runcmd = ["python", "-u", path] if path.endswith(".py") else [path]
    runcmd.extend(args)
...
def client_thread_runner(client_body_func, port, initial_timeout=0.1):
    ...
    tread = threading.Thread(target=socket_reader, args=(sockobj, readq, exit_event))
    tread.start()
...
def test_main():
    ...
    argparser.add_argument(
        "-p", "--server-port", default=9090, type=int, help="the server listens on this port"
    )
    argparser.add_argument(
        "--timeout-bump",
        default=0.0,
        type=float,
        help="amount of time (in sec) by which to bump the " "timeout between consecutive clients",
    )
    argparser.add_argument(
        "-n",
        "--num-clients",
    ...
    argparser.add_argument("--loop", default=1, type=int, help="launch test in a loop")
    ...
    server_thread = threading.Thread(
        target=server_runner, args=(args.server_path, [str(args.server_port)], stop_event)
    )
    server_thread.start()
    time.sleep(0.3)
    ...
    for i in range(args.loop):
        ...
        for i in range(args.num_clients):
            tester_thread = threading.Thread(
                target=client_thread_runner, args=(next(client_iter), args.server_port, TIMEOUT)
            )
            tester_thread.start()

...
int32_t utf8_is_continuation(char c) { return (c & 0xc0) == 0x80; }

int32_t utf8_is_single_byte(char *c) { return (c[0] & 0x80) == 0x0; }

int32_t utf8_is_double_byte(char *c) { return (c[0] & 0xe0) == 0xc0 && utf8_is_continuation(c[1]); }
...
int32_t utf8_is_quadruple_byte(char *c) {
  return (c[0] & 0xf8) == 0xf0 && utf8_is_continuation(c[1]) && utf8_is_continuation(c[2]) &&
         utf8_is_continuation(c[3]);
}
...
int32_t utf8_validate(char *s) {
  ...
  while (i < len) {
    ptrdiff_t num_bytes = utf8_num_bytes(s + i);
    if (num_bytes) {
      i += num_bytes;
    } else {
      return 0;
...
char *utf8_replace(char *needle, char *replace, char *haystack) {
  size_t len_replace = strlen(replace), len_needle = strlen(needle), len = strlen(haystack);
...
char *utf8_replace_all(char *needle, char *replace, char *haystack) {
  char *new_string = utf8_replace(needle, replace, haystack), *old_new_string = NULL;
...
// the length here is the wanted length of the string, not including the
...
char *utf8_escape_null_bytes(const char *s, size_t num) {
  ...
  size_t num_null_bytes = 0, num_from_s = 0;
